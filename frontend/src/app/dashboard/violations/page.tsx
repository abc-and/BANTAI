"use client";
import { useEffect, useState } from "react";
import api from "@/lib/api";
import { Violation } from "@/types";

export default function ViolationsPage() {
  const [violations, setViolations] = useState<Violation[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    api.get("/violations").then((res) => {
      setViolations(res.data);
      setLoading(false);
    }).catch(() => setLoading(false));
  }, []);

  const statusColor: Record<string, string> = {
    PENDING: "bg-amber-500/15 text-amber-400 border-amber-500/25",
    RESOLVED: "bg-emerald-500/15 text-emerald-400 border-emerald-500/25",
    DISMISSED: "bg-slate-500/15 text-slate-400 border-slate-500/25",
  };

  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-white">Violations</h1>
          <p className="text-slate-400 text-sm mt-1">Manage all traffic violations</p>
        </div>
        <button className="px-4 py-2 rounded-xl bg-[#1a8fd1] hover:bg-[#146da3] text-white text-sm font-medium transition-all">
          + New Violation
        </button>
      </div>

      <div className="bg-[#0b1a2e] border border-[rgba(26,143,209,0.1)] rounded-2xl overflow-hidden">
        {loading ? (
          <div className="text-center text-slate-500 py-16">Loading...</div>
        ) : violations.length === 0 ? (
          <div className="text-center text-slate-500 py-16">No violations found</div>
        ) : (
          <table className="w-full">
            <thead>
              <tr className="border-b border-[rgba(26,143,209,0.1)]">
                {["Plate No.", "Type", "Description", "Location", "Fine", "Status", "Date"].map((h) => (
                  <th key={h} className="px-4 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">{h}</th>
                ))}
              </tr>
            </thead>
            <tbody className="divide-y divide-[rgba(26,143,209,0.05)]">
              {violations.map((v) => (
                <tr key={v.id} className="hover:bg-slate-800/30 transition-colors">
                  <td className="px-4 py-3 text-sm text-white font-medium">{v.jeepney?.plateNumber}</td>
                  <td className="px-4 py-3 text-sm text-slate-300">{v.type.replace(/_/g, " ")}</td>
                  <td className="px-4 py-3 text-sm text-slate-400 max-w-[200px] truncate">{v.description}</td>
                  <td className="px-4 py-3 text-sm text-slate-400">{v.location || "—"}</td>
                  <td className="px-4 py-3 text-sm text-slate-300">{v.fineAmount ? `₱${v.fineAmount}` : "—"}</td>
                  <td className="px-4 py-3">
                    <span className={`px-2 py-0.5 rounded-md text-xs font-medium border ${statusColor[v.status] ?? statusColor.PENDING}`}>
                      {v.status}
                    </span>
                  </td>
                  <td className="px-4 py-3 text-sm text-slate-500">{new Date(v.issuedAt).toLocaleDateString()}</td>
                </tr>
              ))}
            </tbody>
          </table>
        )}
      </div>
    </div>
  );
}