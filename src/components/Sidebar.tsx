        ...
        <>
            {showLogout && (
                <div className="fixed inset-0 z-[2000] flex items-center justify-center">
                    ...
                    <div className="relative z-10 w-80 rounded-2xl bg-card border border-border-custom p-6 shadow-2xl">
                        <h2 className="text-foreground font-bold text-center mb-4">Sign out of BANT.AI?</h2>
            ...
            >
                ...
                <div className="relative h-16 border-b border-border-custom bg-sidebar z-50">
                    <div
                        className={`absolute left-1/2 -translate-x-1/2 z-60 transition-all duration-300 pointer-events-none ${collapsed ? "w-10 h-10 top-3" : ""
                            }`}
                        style={!collapsed ? {
                            height: "120px",
                            width: "250px",
                    ...
                    {filteredNav.map((item) => {
                        ...
                        return (
                            <Link key={item.href} href={item.href}
                                ...
                                className={`group flex items-center gap-3 px-3 py-3 rounded-xl font-medium transition-all duration-200 ${collapsed ? "justify-center" : ""} ${active ? "text-white shadow-md shadow-blue-500/20 bg-linear-to-br from-indigo-600 to-blue-700" : "text-foreground/70 hover:bg-blue-50 dark:hover:bg-blue-900/20 hover:text-blue-600"}`}
                            >
                                <span className="shrink-0">{item.icon}</span>
                                {!collapsed && <span className="text-sm truncate">{item.label}</span>}
                                {!collapsed && active && <div className="ml-auto w-1.5 h-1.5 rounded-full bg-white shrink-0" />}
                            </Link>
                        );
                    })}
                </nav>
                ...
                {/* Theme Toggle: bg-background mx-2 */}
                <div className={`px-4 py-3 flex items-center justify-center gap-2 border-t border-border-custom ${collapsed ? "flex-col" : "bg-background/50 mx-2 my-2 rounded-2xl"}`}>
                    <button
                        onClick={() => theme !== 'light' && toggleTheme()}
                        className={`flex items-center gap-2 p-2 rounded-xl transition-all duration-200 ${theme === 'light'
                    ...
                    >
                        ...
                        {!collapsed && <span className="text-[10px] font-bold tracking-widest">LIGHT</span>}
                    </button>

                    {!collapsed && <div className="w-px h-4 bg-border-custom mx-1" />}

                    <button
                        onClick={() => theme !== 'dark' && toggleTheme()}
                        className={`flex items-center gap-2 p-2 rounded-xl transition-all duration-200 ${theme === 'dark'
                    ...
                    >
                        ...
                        {!collapsed && <span className="text-[10px] font-bold tracking-widest uppercase">DARK</span>}
                    </button>
                </div>

                {/* User Section: border-border-custom bg-background */}
                <div className={`p-4 border-t border-border-custom bg-background/30 ${collapsed ? "flex justify-center" : ""}`}>
                    {collapsed ? (
                        ...
                        <div className="flex items-center gap-3 px-2 py-2 rounded-xl">
                            ...
                            <div className="flex-1 min-w-0">
                                <p className="text-sm font-bold text-foreground truncate leading-tight">{user?.firstName} {user?.lastName}</p>
                                <p className="text-xs font-medium text-foreground/50 mt-0.5">{user?.role}</p>
                            </div>
                            <button onClick={() => setShowLogout(true)}
                                className="p-2 rounded-xl text-foreground/40 hover:text-rose-600 hover:bg-rose-50 dark:hover:bg-rose-900/20 transition-all" title="Sign Out">
